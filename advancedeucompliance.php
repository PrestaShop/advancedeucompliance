...
class Advancedeucompliance extends Module
{
    ...
    public function __construct(Core_Foundation_Database_EntityManager $entity_manager, Core_Foundation_FileSystem_FileSystem $fs, Core_Business_Email_EmailLister $email)
    {
        ...
        $this->description =
            $this->l('This module helps European merchants to get compliant with their countries e-commerce laws.');
    ...
    {
        return parent::install() && $this->loadTables() && $this->registerHook('header') &&
               $this->registerHook('displayProductPriceBlock') && $this->registerHook('overrideTOSDisplay') &&
               $this->registerHook('actionEmailAddAfterContent') && $this->registerHook('advancedPaymentOptions') &&
               $this->createConfig();
    }

    public function uninstall()
    {
        return parent::uninstall() && $this->dropConfig() && $this->unloadTables();
    }
    ...
    public function hookAdvancedPaymentOptions($param)
    {
        ...
        Media::addJsDef(array('aeuc_tos_err_str' => $this->l('You need to agree to our Terms Of Service (ToS) before going further!', 'advancedeucompliance')));
        Media::addJsDef(array('aeuc_submit_err_str' => $this->l('Something went wrong. If the problem persists, contact us.', 'advancedeucompliance')));
    ...
    public function hookActionEmailAddAfterContent($param)
    {
        ...
        $final_content =
            $this->context->smarty->fetch($this->local_path . 'views/templates/hook/hook-email-wrapper.tpl');
    ...
    public function hookHeader($param)
    {
        if (isset($this->context->controller->php_self) &&
            ($this->context->controller->php_self === 'index' || $this->context->controller->php_self === 'product')
        ) {
    ...
    public function hookOverrideTOSDisplay($param)
    {
        ...
        if (!$has_tos_override_opt || !$cms_page_associated instanceof CMSRole ||
            (int)$cms_page_associated->id_cms == 0
        ) {
            return false;
        }
        ...
        $this->context->smarty->assign(array('checkedTOS' => $checkedTos, 'link_conditions' => $link_conditions,
            ...
            if ((bool)Configuration::get('AEUC_LABEL_SHIPPING_INC_EXC') === true) {
                ...
                if (!$product->is_virtual) {
                    ...
                    if (isset($cms_page_associated->id_cms) && $cms_page_associated->id_cms != 0) {
                        ...
                        $cms_revocations =
                            $cms_repository->i10nFindOneById($cms_ship_pay_id, $this->context->language->id, $this->context->shop->id);
                        $is_ssl_enabled = (bool)Configuration::get('PS_SSL_ENABLED');
                        $link_ship_pay =
                            $this->context->link->getCMSLink($cms_revocations, $cms_revocations->link_rewrite, $is_ssl_enabled);
    ...
    protected function _postProcess()
    {
        ...
        $post_keys_complex = array('AEUC_legalContentManager', 'AEUC_emailAttachmentsManager'
        );
    ...
    protected function renderFormLabelsManager()
    {
        ...
        $helper->tpl_vars =
            array('fields_value' => $this->getConfigFormLabelsManagerValues(), /* Add values for your inputs */
    ...
    protected function getConfigFormLabelsManager()
    {
        return array('form' => array('legend' => array('title' => $this->l('Labels', 'advancedeucompliance'),
                                                       'icon' => 'icon-tags',
        ), 'input' => array(array('type' => 'text', 'lang' => true,
                                  'label' => $this->l('Estimated delivery time label (available products)', 'advancedeucompliance'),
                                  'name' => 'AEUC_LABEL_DELIVERY_TIME_AVAILABLE', 'desc' => $this->l('Indicate the estimated delivery time for your in-stock products.
						Leave the field empty to disable.', 'advancedeucompliance'),
                            ), array('type' => 'text', 'lang' => true,
                                     'label' => $this->l('Estimated delivery time label (out-of-stock products)', 'advancedeucompliance'),
                                     'name' => 'AEUC_LABEL_DELIVERY_TIME_OOS', 'desc' => $this->l('Indicate the estimated delivery time for your out-of-stock products.
						Leave the field empty to disable.', 'advancedeucompliance'),
                            ), array('type' => 'switch',
                                     'label' => $this->l('\'Before\' Base price label', 'advancedeucompliance'),
                                     'name' => 'AEUC_LABEL_SPECIFIC_PRICE', 'is_bool' => true,
                                     'desc' => $this->l('When a product is on sale, displays the base price with a \'Before\' label.', 'advancedeucompliance'),
                                     'values' => array(array('id' => 'active_on', 'value' => true,
                                                             'label' => $this->l('Enabled', 'advancedeucompliance')
                                                       ), array('id' => 'active_off', 'value' => false,
                                                                'label' => $this->l('Disabled', 'advancedeucompliance')
                                                       )
                                     ),
                            ), array('type' => 'switch',
                                     'label' => $this->l('Tax \'inc./excl.\' label', 'advancedeucompliance'),
                                     'name' => 'AEUC_LABEL_TAX_INC_EXC', 'is_bool' => true, 'desc' => $this->l('Display whether the tax is included next to the product price
						(\'Tax included/excluded\' label).', 'advancedeucompliance'),
                                     ...
                                     'name' => 'AEUC_LABEL_SHIPPING_INC_EXC', 'is_bool' => true, 'desc' => $this->l('Display whether the shipping fees are included, next to the product
						price (\'Shipping fees included / excluded\').', 'advancedeucompliance'), 'hint' => $this->l('If enabled, make sure the Shipping terms are associated with a CMS page
						below (Legal Content Management). The label will link to this content.', 'advancedeucompliance'),
                                     ...
                                     'name' => 'AEUC_LABEL_WEIGHT', 'is_bool' => true,
                                     ...
                                     'name' => 'AEUC_LABEL_REVOCATION_TOS', 'is_bool' => true, 'desc' => $this->l('Include content from the Revocation Terms CMS page within the
						Terms of Services (ToS). ', 'advancedeucompliance'), 'hint' => $this->l('If enabled, make sure the Revocation Terms are associated with a CMS page
						below (Legal Content Management).', 'advancedeucompliance'), 'disable' => true,
                                     'values' => array(array('id' => 'active_on', 'value' => true,
                                                             'label' => $this->l('Enabled', 'advancedeucompliance')
                                                       ), array('id' => 'active_off', 'value' => false,
                                                                'label' => $this->l('Disabled', 'advancedeucompliance')
                            ...
                            array('type' => 'switch', 'label' => $this->l('\'From\' price label (when combinations)'),
                                  'name' => 'AEUC_LABEL_COMBINATION_FROM', 'is_bool' => true,
						...
						that the final price may be higher.', 'advancedeucompliance'), 'disable' => true,
                                  'values' => array(array('id' => 'active_on', 'value' => true,
                                                          'label' => $this->l('Enabled', 'advancedeucompliance')
                                                    ), array('id' => 'active_off', 'value' => false,
                                                             'label' => $this->l('Disabled', 'advancedeucompliance')
        ...
        ), 'submit' => array('title' => $this->l('Save', 'advancedeucompliance'),
        ),
        ),
        );
    }
    ...
    protected function getConfigFormLabelsManagerValues()
    {
        ...
        foreach ($langs as $lang) {
            ...
            $delivery_time_available_values[$tmp_id_lang] =
                Configuration::get('AEUC_LABEL_DELIVERY_TIME_AVAILABLE', $tmp_id_lang);
            $delivery_time_oos_values[$tmp_id_lang] = Configuration::get('AEUC_LABEL_DELIVERY_TIME_OOS', $tmp_id_lang);
        }

        return array('AEUC_LABEL_DELIVERY_TIME_AVAILABLE' => $delivery_time_available_values,
    ...
    protected function renderFormFeaturesManager()
    {
        ...
        $helper->tpl_vars =
            array('fields_value' => $this->getConfigFormFeaturesManagerValues(), /* Add values for your inputs */
                  'languages' => $this->context->controller->getLanguages(),
                  'id_language' => $this->context->language->id,
            );
    ...
    {
        return array('form' => array('legend' => array('title' => $this->l('Features'), 'icon' => 'icon-cogs',
        ), 'input' => array(array('type' => 'switch', 'label' => $this->l('Enable \'Tell A Friend\' feature'),
                                  'name' => 'AEUC_FEAT_TELL_A_FRIEND', 'is_bool' => true, 'desc' => $this->l('Make sure you comply with your local legislation before enabling:
						it can be regarded as an unsolicited commercial email.'), 'hint' => $this->l('If enabled, the \'Send to a Friend\' module allows customers to send to a
						friend an email with a link to a product\'s page.', 'advancedeucompliance'),
                            ...
                            ), array('type' => 'switch', 'label' => $this->l('Enable \'Reordering\' feature'),
                                     'hint' => $this->l('If enabled, the \'Reorder\' option allows customers to reorder in one
						click from their Order History page.', 'advancedeucompliance'), 'name' => 'AEUC_FEAT_REORDER',
                                     'is_bool' => true, 'desc' => $this->l('Make sure you comply with your local legislation before enabling:
						it can be regarded as inertia selling.', 'advancedeucompliance'),
                            ...
                            ), array('type' => 'switch', 'label' => $this->l('Enable \'Advanced checkout page\''),
                                     'hint' => $this->l('The advanced checkout page displays the following sections: payment methods,
						address summary, ToS agreement, cart summary, and an \'Order with Obligation to Pay\' button.', 'advancedeucompliance'),
                                     'name' => 'AEUC_FEAT_ADV_PAYMENT_API', 'is_bool' => true, 'desc' => $this->l('To address some of the latest European legal requirements,
						...
						etc) in one single page.', 'advancedeucompliance'),
                                     ...
                                     'name' => 'PS_ATCP_SHIPWRAP', 'is_bool' => true,
                                     'values' => array(array('id' => 'active_on', 'value' => true,
                                                             'label' => $this->l('Enabled')
                                                       ), array('id' => 'active_off', 'value' => false,
                                                                'label' => $this->l('Disabled')
    ...
    {
        return array('AEUC_FEAT_TELL_A_FRIEND' => Configuration::get('AEUC_FEAT_TELL_A_FRIEND'),
    ...
    protected function renderFormLegalContentManager()
    {
        ...
        foreach ($cms_roles as $cms_role) {
            ...
            $cms_roles_assoc[(int)$cms_role->id] =
                array('id_cms' => (int)$cms_role->id_cms, 'page_title' => (string)$assoc_cms_name,
        ...
        $this->context->smarty->assign(array('cms_roles_assoc' => $cms_roles_assoc, 'cms_pages' => $cms_pages,
                                             'form_action' => '#',
                                             'add_cms_link' => $this->context->link->getAdminLink('AdminCMS')
                                       ));
        $content =
            $this->context->smarty->fetch($this->local_path . 'views/templates/admin/legal_cms_manager_form.tpl');
        return $content;
    }
    ...
    protected function renderFormEmailAttachmentsManager()
    {
        ...
        if (count($cms_roles_associated) != count($cms_roles_full)) {
            $incomplete_cms_role_association_warning = $this->displayWarning($this->l('Your legal content is not linked to any CMS page yet (see above section).
        ...
        foreach ($cms_roles_associated as $role) {
            ...
            $legal_options[$role->name] =
                array('name' => $cms_roles_aeuc[$role->name], 'id' => $role->id, 'list_id_mail_assoc' => $clean_list
                );
        }
        ...
        $this->context->smarty->assign(array('has_assoc' => $cms_roles_associated,
        ...
        $content =
            $this->context->smarty->fetch($this->local_path . 'views/templates/admin/email_attachments_form.tpl');
